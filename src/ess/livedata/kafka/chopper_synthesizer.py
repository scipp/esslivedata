...
def _make_chopper_cascade_message() -> Message[LogData]:
    ...
    The value is unused by the workflow — only its arrival drives a recompute
    (the trigger is the LUT job's ``allow_bypass`` primary stream).
    """
    now = Timestamp.now()
    return Message(
        timestamp=now,
...
def _make_delay_setpoint_message(
    ...
    return Message(
        ...
        stream=StreamId(kind=StreamKind.LOG, name=delay_setpoint_stream(chopper)),
...
class ChopperSynthesizer(MessageSource[Message]):
    ...
    - ``chopper_names`` non-empty: per-chopper plateau detection on delay,
      ...
      ``chopper_cascade`` emission once all choppers are locked.
    """
    ...
    def __init__(
        ...
        *,
        ...
        self._delay_streams = {
            delay_readback_stream(name): name for name in self._chopper_names
        }
        self._speed_streams = {
            speed_setpoint_stream(name): name for name in self._chopper_names
        }
        ...
        if self._chopper_names:
            logger.info(
                ...
                choppers=list(self._chopper_names),
