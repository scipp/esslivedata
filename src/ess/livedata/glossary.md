...
- **Primary / auxiliary / context data** — *primary* streams
  ...
  streams parametrize the workflow graph via `set_context` and gate the job
  (ADR 0002/0003). A context stream whose absence is a representable state
  (the ROI request) declares that state as its `ContextBinding.default`: the
  gate opens on the first tick with that value instead of waiting for a
  producer.
