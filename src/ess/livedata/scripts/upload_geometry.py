...
3. Upload the verified file::
       ...
       python -m ess.livedata.preprocessors.upload_geometry \\
           geometry-<instrument>-<date>.nxs

4. Add the printed registry entry to ``_registry`` in
   ``src/ess/livedata/preprocessors/detector_data.py`` and commit.
...
def main(filepath: str) -> None:
    ...
    print(f"Uploading {path.name} to release '{_RELEASE_TAG}'...")
    _upload(path)

    print(
        f"\nDone. Add this line to _registry in"
        f" src/ess/livedata/preprocessors/detector_data.py:\n\n"
        f"    '{path.name}': 'md5:{md5}',"
    )
...
if __name__ == '__main__':
    ...
    parser = argparse.ArgumentParser(
        ...
        epilog=(
            ...
            '  3. python -m ess.livedata.preprocessors.upload_geometry'
            ' geometry-<instrument>-<date>.nxs\n'
            '  4. Add the printed registry entry to _registry in\n'
            '     src/ess/livedata/preprocessors/detector_data.py and commit'
        ),
