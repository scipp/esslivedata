...
"""Service that computes a wavelength lookup table from chopper setpoints.
...
wavelength lookup-table workflow once.
"""
...
def make_wavelength_lut_service_builder(
    *,
    instrument: str,
    dev: bool = True,
    ...
    scoped = scope_stream_mapping(instrument_config, stream_mapping, 'wavelength_lut')
    ...
    preprocessor_factory = WavelengthLutHandlerFactory(instrument=instrument_config)
...
def main() -> NoReturn:
    runner = DataServiceRunner(
        pretty_name='Wavelength Lookup Table',
        make_builder=make_wavelength_lut_service_builder,
    )
    runner.run()
