...
class PixellatedMonitorOutputs(WorkflowOutputsBase):
    """Outputs for pixellated monitor workflows.

    Includes the same histogram and ratemeter outputs as regular monitors,
    plus a per-pixel counts output.
    """
    ...
    counts_per_pixel: sc.DataArray = pydantic.Field(
        default_factory=lambda: sc.DataArray(
            sc.zeros(dims=['detector_number'], shape=[0], unit='counts'),
        ),
...
def register_pixellated_monitor_workflow_specs(
    instrument: Instrument,
    source_names: list[str],
    params: type[MonitorDataParams] = MonitorDataParams,
) -> SpecHandle | None:
    """
    Register pixellated monitor workflow specs.
    ...
    params
        Parameter model class for the workflow.
    ...
    return instrument.register_spec(
        ...
        name='pixellated_monitor',
        version=1,
        title="Pixellated beam monitor data",
        description=(
            "Histogrammed beam monitor data with per-pixel counts. "
            "Provides the same TOA histogram as regular monitors plus a "
            "counts-per-pixel spatial view."
        ),
        source_names=source_names,
        params=params,
        outputs=PixellatedMonitorOutputs,
    )
