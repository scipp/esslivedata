...
def _make_1d_template() -> sc.DataArray:
    """Create an empty 1D template for I(Q) output."""
    return sc.DataArray(
        sc.zeros(dims=['Q'], shape=[0], unit='1/cm'),
        coords={'Q': sc.arange('Q', 0, unit='1/angstrom')},
    )


def _make_0d_template() -> sc.DataArray:
    """Create an empty 0D template for scalar output."""
    return sc.DataArray(sc.scalar(0.0, unit='dimensionless'))
...
class IofQOutputs(WorkflowOutputsBase):
    ...
    i_of_q: sc.DataArray = pydantic.Field(
        default_factory=_make_1d_template,
        title='I(Q)',
