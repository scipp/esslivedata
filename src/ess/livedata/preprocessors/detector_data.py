...
class DetectorPreprocessorFactory(JobBasedPreprocessorFactoryBase):
    """
    Factory for detector data preprocessors.

    Preprocessors are created based on the instrument name in the message key which
    should identify the detector name. Depending on the configured detector views a
    NeXus file with geometry information may be required to setup the view.
...
# DetectorPreprocessorFactory will fall back to use the detector_number configured in
# the detector view config.
...
_registry = {
    ...
    'geometry-odin-2026-09-04.nxs': 'md5:741f310859d93dc38e1839517aebf639',
