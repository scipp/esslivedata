...
class LogData:
    ...
    Decouples our preprocessors from upstream schema changes. This also simplifies
...
class LatestValueAccumulator(Accumulator[sc.DataArray, sc.DataArray]):
    """
    Context accumulator that keeps only the latest value.

    Implements the Accumulator protocol (add/get/clear) for use in the preprocessor
    layer. For use with StreamProcessor workflows, use LatestValue instead.
