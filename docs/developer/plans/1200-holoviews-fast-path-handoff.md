...
Everything below was measured in this devcontainer. The question this hands off:
**can the remaining per-layer repaint cost be removed in HoloViews itself, in a
shape upstream would accept, instead of as a private-API workaround in
esslivedata?** Two upstream-shaped fixes have already landed on a fork branch;
the next step needs a decision (§5).
...
| | |
|---|---|
| esslivedata worktree | `/workspace/esslivedata/.worktrees/1200-holoviews-fast-path`, branch `1200-holoviews-fast-path` (no source changes yet, harness in `.scratch/1200/`) |
...
All three branches are pushed to the fork (`SimonHeybrock/holoviews`):
...
per layer per frame). That is the two chained `.opts()` wrappers the app stacks
(`plots.py` `DefaultPresenter.present`, `widgets/cell.py` `_compose_plot`) plus
the compositor's own `Dynamic`.
...
So the ranking of what is left, per layer per frame, is now:

| target | worth | risk |
|---|---|---|
| per-frame `.opts()` re-application (2 wrappers) | ~2.6 ms + GC relief | — |
...
2. **esslivedata: drop the two per-frame `.opts()` wrappers.** Style the shared
   element once on the ingestion thread (it is already computed once for all
   sessions) and keep only the per-session hooks — which can ride inside the
   typed `Options` the plotters already declare (verified: HoloViews rejects
   mixing typed `Options` with flat keywords when applied to an element, so the
   hook must go *inside* the per-type `Options`, not alongside them). Same win,
   no upstream dependency, but it is app-side refactoring of `plots.py` and
   `widgets/cell.py`, and it must not leak one session's hooks into another's.
...
Recommendation from the measurements: **2, then 1** — the app change is where the
measured cost actually is and it is ours to make, and doing it first tells us
exactly what an upstream `.opts()` fix has to preserve.
...
| `repaint_bench.py`, `opts_bench.py`, `fastpath_probe.py` | superseded by `bench.py`; keep for the styling-declaration variants |
...
**#1199** (per-frame `.opts()` wrappers — now known to be the dominant remaining
cost, see §4), **#1200** (this work), **#1201** (models per cell), **#1202**
...
(`DefaultPresenter.present`, `Plotter.style_opts`), `dashboard/widgets/cell.py`
(`_compose_plot`, the second `.opts()` wrapper), `dashboard/cell_autoscale.py`
