...
Consolidate today's scattered schema surface into `ess.livedata.schemas` — a pydantic + stdlib subpackage with an enforced dependency boundary, designed so it could later be extracted to a standalone `ess.schemas` in the scipp monorepo.

Headline decisions:

- **Pydantic + stdlib only** inside `schemas/`. No scipp.
- **`ArraySpec` replaces `sc.DataArray`** as the output-template representation.
- **`WorkflowId` is `(instrument, name, version)`.** `namespace` leaves identity; UI grouping moves to a free-form `WorkflowSpec.group`.
- **Discovery by filesystem convention.** Instruments are subpackages under `schemas/instruments/`, no explicit registry.
- **Single consolidated package**, not one per technique.
- **Extraction to `ess.schemas` gated on a second concrete consumer**, not done on speculation.
- **Natural endpoint** (out of scope here, not foreclosed): esslivedata as a streaming + UI adapter over `ess.schemas` + `ess.reduce`.

## At a glance

### Architecture (endpoint — this plan lands only the left-hand box, in-repo)

```
┌──────────────────────────────────────────────────────────────┐
│ ess.livedata                                                  │
│   streaming adapter + dashboard + runtime catalog             │
│   (Kafka, accumulators, orchestration, WorkflowFactory)       │
└────────┬─────────────────────────────────┬────────────────────┘
         │ imports                         │ imports
         ▼                                 ▼
┌───────────────────────────┐   ┌──────────────────────────────┐
│ ess.schemas               │   │ ess.reduce                   │
│   pydantic + stdlib       │◀──│   scipp + sciline            │
│   vocabulary, validators  │   │   transforms (batch + live)  │
│   WorkflowSpec, ArraySpec │   │                              │
└───────────────────────────┘   └──────────────────────────────┘
```

Extraction to `ess.schemas` and `ess.reduce` absorption are future work. This plan carves out the subpackage inside `ess.livedata` with the dependency boundary already enforced. See *Long-term direction* and *Risks and operational constraints*.

### Module layout

```
src/ess/livedata/schemas/
├── __init__.py       # list_instruments, get_instrument, get_workflows
...
Top level is framework; everything under `instruments/` is a plugin. Adding an instrument = adding one subpackage — no registration list, no `__init__.py` edit elsewhere.

### Catalog model

```
SCHEMA CATALOG                          RUNTIME CATALOG
ess.schemas (universe declared)          WorkflowFactory (this deployment serves)
┌─────────────────────────┐
│  dream/powder/v1        │── factory registered ──▶   dream/powder/v1
│  dream/monitor/v1       │── factory registered ──▶   dream/monitor/v1
│  dream/detector/v1      │         ✗ (not served)
│  loki/sans/v1           │         ✗
│  loki/monitor/v1        │         ✗
│  ...                    │
└─────────────────────────┘
...
Dashboard (in-process) resolves the runtime catalog by constructing its own `WorkflowFactory`. Out-of-process frontends would need a published `WorkflowId` set — not in this plan (see *Schema catalog vs. runtime catalog*).
...
Out of scope for this work, but recording because the plan's structure should not silently foreclose it. The direction above implies esslivedata as a **streaming + UI adapter** layered over shared neutron-data libraries, not as a full home-grown reduction implementation. The smaller downstream design choices (where new detector views go, what tests live where, how accumulators factor) get simpler once the long-term answer is explicit.

The likely endpoint the discussion around this plan keeps converging on:

- **`ess.schemas`** — pydantic + stdlib vocabulary, eventually in the `scipp/ess` monorepo alongside technique packages. Serves esslivedata's dashboard, technique-package ipywidget UIs, CLI wrappers, potentially NICOS.
...
Most of what's below applies after extraction to a standalone `ess.schemas`, not during the in-repo consolidation this plan implements. Framed concretely so the team can weigh them before committing to extraction, not after the first incident forces the conversation. Each is also an argument to delay extraction until the benefit is concrete, and to front-load the design decisions that make extraction survivable.
...
### Output templates without scipp (`ArraySpec`)
...
`ess.livedata.schemas` (eventually `ess.schemas`) is a *vocabulary*: the universe of declared workflow specs across all instruments. A concrete esslivedata backend registers factories for a subset — that subset is the **runtime catalog**. (See the diagram in *At a glance*.)
...
### "Schemas" means vocabulary + construction helpers
...
Single type, auto-discovery via filesystem convention. Instruments are the subpackages under `schemas/instruments/`; no explicit registration list. Each `schemas/instruments/<inst>/__init__.py` builds its `InstrumentSpec` eagerly on first import and exposes it as a module-level `spec`. Import caching means subsequent `get_instrument(name)` calls return the same object without rebuilding — Python's module cache *is* the registry.

Under the pydantic+stdlib dependency rule, importing an instrument subpackage is sub-millisecond.

<details>
<summary><b>InstrumentSpec and discovery API — code sketch</b></summary>
...
<details>
<summary><b>Per-area migration details</b></summary>
...
Design and packaging non-scopes, consolidated:

- **Publication as a standalone PyPI package.** The dependency boundary is real at the end of this work; publishing is a separate design conversation — see *Extraction discipline*.
- **JSON Schema emission.** Cheap to add once the surface is scipp-free; add when a consumer (NICOS) needs it.
- **Movement of parameter models into `ess.reduce` / technique packages.** External coordination; see *Long-term direction*.
- **Rethink of `target_keys` / `accumulators` / `dynamic_keys`.** Runtime-side, untouched.
- **New two-phase `register_spec` / `attach_factory` mechanism.** Current decorator pattern continues; only the location of the schema half changes.
- **Schema-layer mechanism for source-dependent parameters (#680).** Workflows whose parameter validity, defaults, or ranges genuinely depend on which source is selected are split into multiple `WorkflowSpec`s with disjoint `source_names` subsets, each carrying its own `params` model (typically subclassing a shared base). The DREAM monitor TOF case (`monitor_cave` vs. `monitor_bunker`) becomes two specs sharing a common `DreamMonitorDataParams` base. No `params_overrides`, no source-aware validator context, no spec-level hook — structural separation via splitting is the design answer.
