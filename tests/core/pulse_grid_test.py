...
"""Tests for PulseGrid and StreamRateEstimator components."""
...
class TestStreamRateEstimator:
    ...
    def test_first_update_sets_rate(self):
        est = StreamRateEstimator()
        est.update(14, 1.0, alpha=0.05)
        assert est.rate_hz == 14.0

    def test_ema_smoothing(self):
        est = StreamRateEstimator()
        est.update(14, 1.0, alpha=1.0)
        est.update(10, 1.0, alpha=0.5)
        assert est.rate_hz == pytest.approx(12.0)

    def test_converged_after_min_batches(self):
        est = StreamRateEstimator()
        for _ in range(MIN_BATCHES_FOR_GATE):
            est.update(14, 1.0, alpha=1.0)
        assert est.converged

    def test_not_converged_before_min_batches(self):
        est = StreamRateEstimator()
        for _ in range(MIN_BATCHES_FOR_GATE - 1):
            est.update(14, 1.0, alpha=1.0)
        assert not est.converged

    def test_integer_rate_rounding(self):
        est = StreamRateEstimator()
        est.update(14, 1.0, alpha=1.0)
