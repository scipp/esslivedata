...
def test_new_dashboard_adopts_running_job_without_restart(
    ...
    Adoption logic is pinned at the seam by the dashboard unit tests, which
    feed synthesized heartbeats. What only the wire can show is that the x5f2
    heartbeats a real service actually emits carry everything the adoption
    needs: a new dashboard, whose in-memory config store holds no record of
    the job, recovers the running job's generation, admits its data, and does
    so without writing a single command (no spurious restart).
    """
    ...
    with DashboardBackend(instrument='dummy', dev=True) as backend_b:
        ...
        # No spurious restart: no command (start or stop) was written.
