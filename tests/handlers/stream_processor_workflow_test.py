...
class TestResetOnContextChange:
    """Tests for reset_on_context_change feature."""

    def test_accumulator_resets_when_tracked_context_changes(
        self, base_workflow_with_context
    ):
        """Changing a tracked context key clears accumulated data."""
        workflow = StreamProcessorWorkflow(
            base_workflow_with_context,
            dynamic_keys={'streamed': Streamed},
            context_keys={'context': Context},
            target_keys={'output': Output},
            accumulators=(ProcessedStreamed,),
            reset_on_context_change=frozenset({'context'}),
        )

        # Set context and accumulate data
        workflow.accumulate({'context': Context(5)}, start_time=0, end_time=100)
        workflow.accumulate({'streamed': Streamed(10)}, start_time=0, end_time=100)

        # Change context — should trigger reset, clearing accumulated streamed data
        workflow.accumulate({'context': Context(99)}, start_time=200, end_time=300)
        workflow.accumulate({'streamed': Streamed(1)}, start_time=200, end_time=300)

        result = workflow.finalize()
        # After reset: context=99, static=2, processed_context=198, streamed=1
        # Output = 1 + 198 = 199
        assert result == {'output': Output(199)}

    def test_no_reset_when_context_unchanged(self, base_workflow_with_context):
        """Sending the same context value should not reset accumulators."""
        workflow = StreamProcessorWorkflow(
            base_workflow_with_context,
            dynamic_keys={'streamed': Streamed},
            context_keys={'context': Context},
        ...
        # Same context value — no reset
        workflow.accumulate({'context': Context(5)}, start_time=100, end_time=200)
        workflow.accumulate({'streamed': Streamed(20)}, start_time=100, end_time=200)

        result = workflow.finalize()
        # Both accumulates count: 10+20=30 streamed, context=5*2=10
        # Output = 30 + 10 = 40, then accumulated: 40 + 10 = 50
        assert result == {'output': Output(50)}

    def test_no_reset_for_untracked_context_key(self, base_workflow_with_context):
        """Changing an untracked context key should not reset accumulators."""
        workflow = StreamProcessorWorkflow(
            base_workflow_with_context,
            dynamic_keys={'streamed': Streamed},
            context_keys={'context': Context},
            target_keys={'output': Output},
            accumulators=(ProcessedStreamed,),
            reset_on_context_change=frozenset(),  # Nothing tracked
        )

        workflow.accumulate({'context': Context(5)}, start_time=0, end_time=100)
        workflow.accumulate({'streamed': Streamed(10)}, start_time=0, end_time=100)

        # Change context — but not tracked, so no reset
        workflow.accumulate({'context': Context(99)}, start_time=200, end_time=300)
        workflow.accumulate({'streamed': Streamed(1)}, start_time=200, end_time=300)

        result = workflow.finalize()
        # Both accumulates count. With new context=99: processed_context=198
        # First: 10+198=208, Second: 208+1=209, Third: 209+198=407
        # Actually let me think more carefully...
        # Accumulate 1: context=5 → set_context(Context=5)
        # Accumulate 2: streamed=10 → accumulate(Streamed=10)
        #   ProcessedStreamed = 10 + ProcessedContext(5*2=10) = 20
        #   Accumulated: 20
        # Accumulate 3: context=99 → set_context(Context=99)
        # Accumulate 4: streamed=1 → accumulate(Streamed=1)
        #   ProcessedStreamed = 1 + ProcessedContext(99*2=198) = 199
        #   Accumulated: 20 + 199 = 219  (no reset!)
        # Finalize: Output = 219
        assert result == {'output': Output(219)}

    def test_reset_clears_time_tracking(self, base_workflow_with_context):
        """Reset on context change should also reset time tracking."""
        workflow = StreamProcessorWorkflow(
            base_workflow_with_context,
            dynamic_keys={'streamed': Streamed},
            context_keys={'context': Context},
        ...
        # Context change resets time tracking
        workflow.accumulate({'context': Context(99)}, start_time=5000, end_time=6000)

        # After reset, start_time should be None, next accumulate sets it fresh
        assert workflow._current_start_time is None

    def test_first_context_value_does_not_trigger_reset(
        self, base_workflow_with_context
    ):
        """The initial context value should not trigger a reset."""
        workflow = StreamProcessorWorkflow(
            base_workflow_with_context,
            dynamic_keys={'streamed': Streamed},
            context_keys={'context': Context},
        ...
        # First context value — should NOT reset (nothing to reset)
        workflow.accumulate({'context': Context(5)}, start_time=0, end_time=100)
        workflow.accumulate({'streamed': Streamed(10)}, start_time=0, end_time=100)

        result = workflow.finalize()
        # context=5*2=10, streamed=10, output=10+10=20
        assert result == {'output': Output(20)}
