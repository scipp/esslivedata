...
"""Tests for the chopperless TOF lookup-table workflow."""
...
def _params(*, neutrons: int = 50_000) -> LookupTableParams:
    p = LookupTableParams()
...
class TestLookupTableComputeWorkflow:
    def test_finalize_without_trigger_raises(self):
        wf = create_chopperless_lookup_table_workflow(params=_params())
    ...
    def test_chopperless_produces_table_with_expected_dims(self):
        ...
        out = wf.finalize()
        assert set(out) == {'lookup_table'}
        table = out['lookup_table']
    ...
    def test_provenance_coords_attached(self):
        params = _params()
        wf = create_chopperless_lookup_table_workflow(params=params)
        wf.accumulate(_trigger(), start_time=0, end_time=1)
        table = wf.finalize()['lookup_table']
    ...
    def test_finalize_caches_result(self):
        wf = create_chopperless_lookup_table_workflow(params=_params())
        wf.accumulate(_trigger(), start_time=0, end_time=1)
        first = wf.finalize()['lookup_table']
        second = wf.finalize()['lookup_table']
    ...
    def test_subclass_marker(self):
        wf = create_chopperless_lookup_table_workflow(params=_params())
        assert isinstance(wf, LookupTableComputeWorkflow)
...
class TestDa00RoundTrip:
    def test_round_trip_preserves_dims_shape_and_provenance_coords(self):
        wf = create_chopperless_lookup_table_workflow(params=_params())
        wf.accumulate(_trigger(), start_time=0, end_time=1)
        original = wf.finalize()['lookup_table']
