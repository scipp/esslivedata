...
class TestComposeNonOverlayableLayers:
    """Composition of cells whose layers cannot share a single figure.

    Tables and Layouts both render as something other than a plain Bokeh
    figure, so they cannot be fused via ``hv.Overlay``. The cell falls back to
    a ``pn.Tabs`` of independent panes instead.
    """

    @staticmethod
    def _add_element_layer(
        plot_grid_tabs,
        plot_data_service,
        *,
        element,
        ...
        """Register a layer whose plotter caches the given hv element."""
        import uuid
        ...
        class _FakePresenter(PresenterBase):
            def present(self, pipe):
                import holoviews as hv

                return hv.DynamicMap(
                    lambda data: self._plotter.get_cached_state(), streams=[pipe]
                )

        class _FakePlotter:
            AUTOSCALE_AXES: frozenset[Axis] = frozenset()
        ...
        config = PlotConfig(
            data_sources={
                PRIMARY: DataSourceConfig(
                    workflow_id=wf, source_names=['bank0'], view_name=view_name
                )
            },
            plot_name=plot_name,
            params=WindowParams(),
        ...
        layer = Layer(
            layer_id=LayerId(uuid.uuid4()), config=config, user_label=user_label
        )
        plotter = _FakePlotter(element)
        plot_data_service.job_started(layer.layer_id, plotter)
        plot_data_service.data_arrived(layer.layer_id)
        state = plot_data_service.get(layer.layer_id)
        session_layer = SessionLayer(
    ...
    def test_layout_layer_with_figure_layer_composes_as_tabs(
        self, plot_grid_tabs, plot_data_service
    ):
        """A Layout cannot be overlaid with another layer.

        ``hv.Overlay([Layout, figure]).collate()`` raises at construction, so a
        cell mixing a Layout-producing layer with a plain figure layer must fall
        back to the tabbed layout rather than the overlay path.
        """
        from uuid import uuid4
        ...
        layout = hv.Curve(np.arange(3)) + hv.Curve(np.arange(3))
        layers = [
            self._add_element_layer(
                plot_grid_tabs,
                plot_data_service,
                element=layout,
                plot_name='lines',
    ...
    def test_tab_titles_prefer_user_label(self, plot_grid_tabs, plot_data_service):
        from uuid import uuid4
        ...
        titles = cell_widget._plot._names
        assert titles[0] == 'My counts'

    def test_duplicate_tab_titles_are_disambiguated(
        self, plot_grid_tabs, plot_data_service
    ):
        """Two layers with the same derived title must get distinct tab titles."""
        from uuid import uuid4
