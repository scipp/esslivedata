...
def _raw_counts() -> sc.DataArray:
    ...
    # Pixel count of `detector_number` in coda_freia_999999_00022031.hdf. The
    # fold is meaningless if it does not cover exactly the pixels the file
    # carries, and the coda file is too large to check in.
    rng = np.random.default_rng(seed=1)
    return sc.DataArray(
        sc.array(
            dims=['detector_number'],
            values=rng.integers(0, 5, size=65536),
            unit='counts',
        )
    )
